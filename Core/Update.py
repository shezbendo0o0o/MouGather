...
class Downloader:
    ...
    def Update():
        ...
        os.chdir(Path)
        if os.path.exists("MouGather_Old"):
            shutil.rmtree("MouGather_Old",)
        if os.name == "nt":
            os.system("rename MouGather MouGather_Old 2>NUL >NUL")
        else:
            os.system("mv MouGather MouGather_Old &>/dev/null")
        os.system("git clone https://github.com/shezbendo0o0o/MouGather")
        choice = int(input(Font.Color.BLUE + "\n[+]" + Font.Color.WHITE +
                     Language.Translation.Translate_Language(filename, "Update", "Choice", "None")))
        if choice == 1:
            os.remove("MouGather_Old")
